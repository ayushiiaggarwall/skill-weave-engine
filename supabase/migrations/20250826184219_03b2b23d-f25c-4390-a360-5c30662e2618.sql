...
-- Optional FK (kept consistent with existing style that avoids many FKs). Add if desired:
-- ALTER TABLE public.course_pricing
--   ADD CONSTRAINT course_pricing_course_fk
...
-- 2) Extend courses with start/end dates and is_active flag (keep existing schema otherwise)
ALTER TABLE public.courses
  ADD COLUMN IF NOT EXISTS start_date date,
  ADD COLUMN IF NOT EXISTS end_date date,
  ADD COLUMN IF NOT EXISTS is_active boolean NOT NULL DEFAULT false;

-- 3) Prepare enrollments to reference courses instead of cohorts (keep cohort_id column for now to avoid breaking existing data)
...
DO $$ BEGIN
  IF EXISTS (
    ...
    DROP TABLE public.pricing_settings;
  END IF;
END $$;
